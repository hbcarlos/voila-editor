...
export class LabShell extends Widget {
    ...
    constructor() {
        ...
        this.currentChanged.connect((sender, args) => {
            ...
            if (oldValue && oldValue instanceof MainAreaWidget) {
                oldValue.content.title.changed.disconnect(this._updateTitlePanelTitle, this);
            }
            // Start watching the title of the new current widget
            if (newValue && newValue instanceof MainAreaWidget) {
                newValue.content.title.changed.connect(this._updateTitlePanelTitle, this);
     ...
     * Widgets must have a unique `id` property, which will be used as the DOM id.
     */
    _addToTitleArea(widget, options) {
        var _a;
        if (!widget.id) {
            console.error('Widgets added to app shell must have unique id property.');
            return;
